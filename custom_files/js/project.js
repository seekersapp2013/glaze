...
Installer.Pages.projectForm.includePackage = function(el, code) {
    var
        ...
        handler = 'onPluginDetails',
        partial = 'project/plugin',
        dataSetId = 'includedPlugins',
...
Installer.Pages.projectForm.requiredPlugins = function(el, code) {
    var
        $el = $(Installer.Pages.projectForm.findIncludeManagerFromEl(el)),
        $list = $el.find('.product-list:first'),
        $empty = $el.find('.product-list-empty:first'),
...
/*
productList = ['RainLab.User','Vdomah.JWTAuth','RainLab.Notify','RainLab.Location','RainLab.UserPlus','Uxms.Userverify','Tiipiik.SmsSender','RainLab.Translate','RainLab.Pages','PanaKour.Translate','Esroyo.UserProfile','Mey.Breadcrumbs','Responsiv.Currency','Initbiz.Money','Responsiv.Uploader','PanaKour.Backup','Jacob.Logbook','Briddle.Pay','SureSoftware.MailLog','Kocholes.BarcodeGenerator','Excodus.TranslateExtended','Clake.UserExtended','Clake.Pusher','Responsiv.Pay','Zainab.SimpleContact'];
//productList = ['RainLab.User'];
$.each(productList, function(index, code){
    var name        =   code.split('.')[1];
    var product = '<a href="javascript:;" data-code="'+code+'"><div class="details"><h5 class="text-overflow">'+name+'</h5></div></a>';
    Installer.Pages.projectForm.requiredPlugins(product, code);
})
//Installer.Pages.projectForm.requiredPlugins(product, 'RainLab.User');
*/
